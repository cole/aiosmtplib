...
async def smtp_protocol(request, raw_preset_server, event_loop):
    ...
    connect_future = event_loop.create_connection(
        lambda: protocol, host=raw_preset_server.hostname,
        port=raw_preset_server.port)

    _, protocol = await asyncio.wait_for(
        connect_future, timeout=1, loop=event_loop)
...
async def test_protocol_connected_check_on_drain_writer(
        disconnected_smtp_protocol):
    with pytest.raises(SMTPServerDisconnected):
